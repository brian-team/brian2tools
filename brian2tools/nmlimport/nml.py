    ...
    try:
        ...
        for segment in segments:
            if segment.parent != None:
                ...
                logger.info("starting segment is {0}".format(formatter(segment)))
...
def generate_morph_object(cell):
    ...
    # validate morphology
    validate_morphology(sorted_segments)

    # generate initial tuple
    seg = sorted_segments[0]
    point = (seg.id, seg.name, seg.proximal.x \
                 , seg.proximal.y, \
             seg.proximal.z, seg.proximal.diameter, \
             -1 if seg.parent == None else seg.parent.segments)
    points = points + (point,)

    # iterate over morphology segments
    for segment in sorted_segments:
        segment_type = get_segment_type(segment)
        point = (segment.id + 1, segment.name, segment.distal.x \
                     , segment.distal.y, \
                 segment.distal.z, segment.distal.diameter, \
                 0 if segment.parent == None else segment.parent.segments + 1)
        points = points + (point,)
...
def load_morph_from_cells(cells, cell_id=None):
    ...
    err= ("The cell id you specified {0} doesn't exist."
          ...
          formatter([cell.id for cell in cells])))
...
def load_morphology(nml_file, cell_id=None):
    ...
    if size(doc.cells) > 1:
        ...
        for cell in doc.cells:
            morphologies[cell.id] = load_morph_from_cells(doc.cells, cell_id=cell.id)
        return morphologies
